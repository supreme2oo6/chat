...
BOT_TOKEN = "7911728544:AAGgZFEEzqcP8SpcPVayvU-bJhTi6lAzUeE"
ADMIN_ID = 1203353320
...
required_channels = []
chat_logs = {}
admin_states = {}  # Для хранения состояний админа при добавлении каналов

# Загрузка данных
def load_data():
    global users_data, required_channels, total_connections
    try:
        ...
        if os.path.exists(CHANNELS_FILE):
            with open(CHANNELS_FILE, 'r', encoding='utf-8') as f:
                data = json.load(f)
                required_channels = data.get('channels', [])
                total_connections = data.get('total_connections', 0)
        logger.info(f"Данные загружены: пользователей {len(users_data)}, каналов {len(required_channels)}")
...
async def show_subscription_requirement(update, context):
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not users_data[user_id].get('registered', False):
        await update.message.reply_text(
    ...
    gender_emoji = {'male': '👨', 'female': '👩', 'none': '🤖'}.get(users_data[user_id]['gender'], '🤖')
...
async def admin_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    male_count = sum(1 for u in users_data.values() if u.get('gender') == 'male')
    female_count = sum(1 for u in users_data.values() if u.get('gender') == 'female')
    none_count = sum(1 for u in users_data.values() if u.get('gender') == 'none')
    banned_count = sum(1 for u in users_data.values() if u.get('banned', False))
...
async def admin_stats_callback(update, context):
    ...
    recent_users = sorted(
        [(uid, data.get('join_date', '')) for uid, data in users_data.items() if data.get('join_date')],
...
async def admin_users_callback(update, context):
    ...
    banned_users = [uid for uid, data in users_data.items() if data.get('banned', False)]
    ...
    if reported_users:
        ...
        for i, (uid, count) in enumerate(reported_users, 1):
            username = users_data.get(uid, {}).get('username', 'н/д')
...
async def admin_add_channel_callback(update, context):
    query = update.callback_query
...
async def admin_list_channels_callback(update, context):
    query = update.callback_query
...
async def remove_specific_channel(update, context, channel_index):
    query = update.callback_query
...
async def admin_remove_all_channels_callback(update, context):
    query = update.callback_query
...
async def admin_ban_user_callback(update, context):
    query = update.callback_query
    user_id = query.from_user.id
...
async def admin_unban_user_callback(update, context):
    query = update.callback_query
    user_id = query.from_user.id
...
async def admin_banned_list_callback(update, context):
    query = update.callback_query
    
    banned_users = [(uid, data) for uid, data in users_data.items() if data.get('banned', False)]
...
async def handle_admin_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == 'waiting_channel_id':
        ...
        if ban_user_id in users_data:
            users_data[ban_user_id]['banned'] = True
            save_data()
            ...
            user_info = users_data[ban_user_id]
            await update.message.reply_text(
        ...
        if unban_user_id in users_data:
            if users_data[unban_user_id].get('banned', False):
                users_data[unban_user_id]['banned'] = False
                save_data()
                
                user_info = users_data[unban_user_id]
                await update.message.reply_text(
...
async def handle_text_commands(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id not in users_data or not users_data[user_id].get('registered', False):
        await update.message.reply_text(
...
async def find_partner(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if waiting_users:
        ...
        user_gender = users_data.get(user_id, {}).get('gender', 'none')
        partner_gender = users_data.get(partner_id, {}).get('gender', 'none')
...
async def report_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if ADMIN_ID:
        try:
            partner_info = users_data.get(partner_id, {})
            await context.bot.send_message(
                chat_id=ADMIN_ID,
